...
int WechaSetTopest(int iParam)
{
	...
	string strPngNoDisturb = strDir + strRes + _T("nodisturb.png");
	if ((_access(strPngNoDisturb.data(), 0)) != 0) {
		cout << _T("Unexpected aborted: can't read resource data:nodisturb.png") << endl;
		return -1;
	}

	string strPngTopest = strDir + strRes + _T("topest.png");
	if ((_access(strPngTopest.data(), 0)) != 0) {
		cout << _T("Unexpected aborted: can't read resource data:topest.png") << endl;
		return -1;
	}

	string strPngSwitchoff = strDir + strRes + _T("wxswitch220723.png");
	if ((_access(strPngSwitchoff.data(), 0)) != 0) {
		cout << _T("Unexpected aborted: can't read resource data:wxswitch220723.png") << endl;
		return -1;
	}

	string strPngSwitchon = strDir + strRes + _T("wxswitchon220723.png");
	if ((_access(strPngSwitchon.data(), 0)) != 0) {
		cout << _T("Unexpected aborted: can't read resource data:wxswitchon220723.png") << endl;
		return -1;
	}
	...
	for (i = 0; i < vtStrLine.size();i++) {
		...
		if (ret) {
			...
			pt.x = rcTemp.left + 680; pt.y = rcTemp.top + 40;
			SetCursorPos(pt.x, pt.y);
			mouse_event(MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
			Sleep(500);
			...
			if ((iParam & 0b1) == 1) {
				matTempl = imread(strPngTopest);
				ZeroMemory(szRc, sizeof(CvRect[UINT_SIZEOFRECT]));
				ret = MatchTemplOnWindow(GetSpecWindow(), matTempl, ptrNULL, szRc, UINT_SIZEOFRECT, 0.9);
				if (!ret) {
					cout << _T("Unexpected aborted: can't find resource data:strPngTopest") << endl;
					exit(1);
				}
				...
				ZeroMemory(szRc, sizeof(CvRect[UINT_SIZEOFRECT]));
				ret = MatchTemplOnWindow(GetSpecWindow(), matTempl, ptrNULL, szRc, UINT_SIZEOFRECT, 0.9, 0);
				if (ret) {
					for (j = 0; j < ret; j++) {
						if (pt.y >(rcTemp.top + szRc[j].y) && pt.y < (rcTemp.top + szRc[j].y + szRc[j].height)) {
							pt.x = rcTemp.right - 40;
							SetCursorPos(pt.x, pt.y);
							mouse_event(MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
							Sleep(500);
						}
			...
			if ((iParam & 0b10) == 2) {
				matTempl = imread(strPngNoDisturb);
				ZeroMemory(szRc, sizeof(CvRect[UINT_SIZEOFRECT]));
				ret = MatchTemplOnWindow(GetSpecWindow(), matTempl, ptrNULL, szRc, UINT_SIZEOFRECT, 0.9);
				if (!ret) {
					cout << _T("Unexpected aborted: can't find resource data:strPngNoDisturb") << endl;
					exit(1);
				}
				pt.x = rcTemp.left + szRc[0].x + szRc[0].width / 2; pt.y = rcTemp.top + szRc[0].y + szRc[0].height / 2;
